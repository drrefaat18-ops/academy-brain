...
def _instantiate_specialist(seed: Seed, root: Path) -> str:
    ...
    body = (
        template
        # Before the bare COURSE pass, so the human-facing TODO markers stay
        # legible as markers rather than becoming `<!-- kids-ev3: ... -->`.
        .replace("<!-- COURSE:", f"<!-- TODO({seed.slug}):")
        .replace(
            _SPECIALIST_HEADER,
        ...
        # Last, so a legitimate domain name containing COURSE or SUBJECT is not
        # mistaken for template syntax after it has been inserted.
